...
public class Player {
    public HashMap<String, Integer> extraPieces;
    public boolean player1;

    public Player(boolean player1){
        super();
        this.extraPieces = new HashMap<String, Integer>();
        this.player1 = player1;
    }

    public void addPiece(String killedPieceRep){
        Integer count = extraPieces.get(killedPieceRep);
        if (count != null) {
            extraPieces.put(killedPieceRep, count + 1);
        }
        else {
            extraPieces.put(killedPieceRep, 1);
        }
    }

    public Piece pieceFromRep(String rep) {
       String lowerRep = rep.toLowerCase();
       if (lowerRep.equals("c")) {
            Chicken newPiece = new Chicken(player1, "c");
            return newPiece;
       }
       else if (lowerRep.equals("l")) {
            Lion newPiece = new Lion(player1, "l");
            return newPiece;
        }
        else if (lowerRep.equals("g")) {
            Giraffe newPiece = new Giraffe(player1, "g");
            return newPiece;
        }
        else if (lowerRep.equals("e")) {
            Elephant newPiece = new Elephant(player1, "e");
            return newPiece;
        }
        else {
            Piece defPiece = new Piece(player1, ".");
            return defPiece;
        }
    }

    public HashMap<String, Integer> getPieces(){
        return extraPieces;
    }
